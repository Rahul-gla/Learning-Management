...
// import { useGetAllPurchasedCoursesQuery } from "@/features/api/purchaseApi"; // Import the new query
import React from "react";
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from "recharts";

const Dashboard = () => {
  const { data, isSuccess, isError, isLoading } = useGetAllPurchasedCoursesQuery(); // Use the new query

  if (isLoading) return <h1>Loading...</h1>;
  if (isError) return <h1 className="text-red-500">Failed to get purchased courses</h1>;

  const purchasedCourses = data?.purchasedCourses || []; // Ensure this matches your API response structure
